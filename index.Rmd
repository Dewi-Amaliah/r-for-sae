...
- Contoh kasus: mengetahui jumlah lulusan SD di seluruh kabupaten di Indonesia berdasarkan jenis kelamin
- Perlu memperhatikan variabel penimbang
...
lulusan_sd <- sak_leveled %>%
  ...
  summarise(jumlah = sum(penimbang, na.rm = TRUE)) %>%
  ungroup()
...
lulusan_sd_joined <- left_join(kode_kab, lulusan_sd, by = c("KODE_KABB" = "kode_kab"))
...
lulusan_sd_wider <- lulusan_sd_joined %>%
  pivot_wider( names_from = "jk", values_from = "jumlah")
...
<div class="alert alert-info">
  <strong>HELP!!!</strong> Siti diminta oleh rekan kerjanya untuk membuat tulisan mengenai <strong>Hustle Culture</strong> di Indonesia. Ia diminta untuk menghitung jumlah penduduk bekerja yang memiliki pekerjaan tambahan berdasarkan status perkawinan dan provinsi.
</div>
...
2. Selain itu, bantu pula Siti untuk mengetahui provinsi mana dengan Hustle Culture paling mencolok.
